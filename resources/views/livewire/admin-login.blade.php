...
mount(function () {
    // Check if regular user (non-admin) is already logged in
    if (Auth::check() && Auth::user()->profile?->role !== 'admin') {
        // Regular user trying to access admin login - show access denied
...
$execLogin = function () {
    ...
    if (!$user->profile || $user->profile->role !== 'admin') {
        throw ValidationException::withMessages([
            'role_error' => 'Anda tidak memiliki akses admin. Silakan hubungi administrator sistem.',
        ]);
    }
...
<div>
    <div class="flex justify-center items-center h-screen bg-gradient-to-b from-[#2C3E50] to-[#95CAFF]">
        <div
            ...
            <div class="forms-container relative w-[50%] text-center">
                <div
                    ...
                    <form wire:submit.prevent="execLogin" class="flex flex-col mx-[50px]" x-data="{ login_field: @entangle('login_field').defer, password: @entangle('password').defer, init() { console.log('Alpine.js x-data initialized for login form'); } }">
                        {{-- INTRODUCTION --}}
                        <div class="text-[#1A1A1A] text-5xl font-Kuunari font-bold text-center sm:text-start">
                            <h1>HALO ADMIN!</h1>
                        </div>
                        <div
                            class="text-[#000000] text-sm font-Poppins text-center sm:text-start justify-center pt-1 mb-5">
                            <p>Masuk dan mulai atur layanan kamu sekarang.</p>
                        </div>
               ...
               shadow-sm has-[:focus]:shadow-md transition-all duration-200">
                        ...
                        <div class="flex justify-center">
                            <button type="submit" :disabled="!login_field || !password" {{-- Button is disabled if login_field OR password is empty --}}
                                class="mt-5 bg-[#011C19] text-Seasalt w-full text-sm py-4 rounded-md font-bold
                   transition duration-300 ease-in-out shadow-lg font-Poppins
                   hover:text-white hover:bg-[#011C19]/60 focus:outline-2
                   disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-[#011C19] disabled:hover:text-Seasalt">
                                MASUK SEKARANG!
                            </button>
                        </div>
                    </form>
            ...
            <div class="intros-container relative left-[50%] w-[50%] text-center">
                <div
                    ...
                    <div
                        class="absolute inset-1/2 -translate-x-1/2 -translate-y-1/2 w-[98%] h-[98%] bg-[#2C3E50] opacity-40 bg-no-repeat bg-center bg-contain z-0 rounded-xl shadow-xl">
                    </div>
