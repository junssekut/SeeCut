        ...
        <div
            ...
            <div class="forms-container relative w-[50%] text-center">
                <div
                    ...
                    <form action="#" class="flex flex-col mx-[50px]">
                        {{-- INTRODUCTION --}}
                        <div class="text-[#6B592E] text-5xl font-Kuunari font-bold text-center sm:text-start">
                            <h1>DAFTAR</h1>
                        </div>
                        <div class="text-[#000000] text-sm font-Poppins text-center sm:text-start justify-center pt-1 mb-5">
                        ...
                        <div
                            ...
                            <label for="nama" class=" flex flex-1 flex-wrap border-none items-center">
...
-1092 2302 -2151 2994 -716 467 -1523 744 -2378 816 -130 10 -499 19 -605 14z" />
                                ...
                                <input type="text" id="nama" name="nama" placeholder="Masukkan username anda"
                                    required
                            ...
                            <div class="w-4 h-4 mx-4 cursor-pointer"
                                ...
                                <span id="openIcon2" class="toggle-icon">
                                    <svg id="eye-konfirmasi-closed" version="1.0" xmlns="http://www.w3.org/2000/svg"
                                        class="w-full h-full pointer-events-none" viewBox="0 0 1181.000000 1181.000000"
...
-58z" />
                                ...
                                <span id="closedIcon2" class="toggle-icon hidden">
                                    <svg id="eye-konfirmasi-open" version="1.0" xmlns="http://www.w3.org/2000/svg"
                                        class="w-full h-full pointer-events-none" viewBox="0 0 1350.000000 1350.000000"
                                        preserveAspectRatio="xMidYMid meet">
...
-43z" />
                            ...
                            <div class="w-4 h-4 mx-4 cursor-pointer"
                                onclick="togglePassword('confirmpassword', 'openIcon1', 'closedIcon1')">
                                <!-- SVG icon MATA -->
...
80 10 195 10 115 0 191 -4 195 -10z" />
    ...
    <div onclick="togglePassword('passwordField1', 'openIcon1', 'closedIcon1')">
        <span id="openIcon1" class="toggle-icon hidden">👁️</span>
        <span id="closedIcon1" class="toggle-icon">🔒</span>
    </div>
</div> --}}
                        </div>
                        <button
...
-1092 2302 -2151 2994 -716 467 -1523 744 -2378 816 -130 10 -499 19 -605 14z" />
                        ...
                        {{-- INPUT PASSWORD --}}
                        <div
                            class="text-[#6B592E] flex justify-between flex-wrap py-4 border-none bg-[#E9BF80] rounded-md items-center cursor-text">
                            <label for="password"
                                class=" flex flex-1 flex-wrap border-none bg-[#E9BF80] rounded-md items-center cursor-text">
                                <div class="w-4 h-4 mx-4 flex items-center justify-center overflow-hidden">
                                    <!-- SVG icon -->
...
-43z" />
                            ...
                            <div class="w-4 h-4 mx-4 cursor-pointer"
                                onclick="togglePassword('passwordlogin', 'openIcon3', 'closedIcon3')">
                                <!-- SVG icon MATA -->
                                <span id="openIcon3" class="toggle-icon">
                                    <svg id="eye-konfirmasi-closed" version="1.0" xmlns="http://www.w3.org/2000/svg"
                                        class="w-full h-full pointer-events-none"
                                        viewBox="0 0 1181.000000 1181.000000" preserveAspectRatio="xMidYMid meet">
...
-58z" />
                                ...
                                <span id="closedIcon3" class="toggle-icon hidden">
                                    <svg id="eye-konfirmasi-open" version="1.0" xmlns="http://www.w3.org/2000/svg"
                                        class="w-full h-full pointer-events-none"
                                        viewBox="0 0 1350.000000 1350.000000" preserveAspectRatio="xMidYMid meet">
...
80 10 195 10 115 0 191 -4 195 -10z" />
                        ...
                        <button
                            class="mt-5 bg-[#6B592E] w-full text-[#FFEDB7] hover:text-white text-sm font-bold py-4 px-10 rounded-md transition duration-300 ease-in-out hover:bg-[#B5964D] focus:outline-2">
                            MASUK SEKARANG!
                        </button>
                    </form>
                </div>
            ...
            <div class="intros-container relative left-[50%] w-[50%] text-center">
                <div
                    ...
                    <img src="{{ asset('assets/images/frenchcrop.jpg') }}"
                        class="absolute inset-0 w-full h-full object-cover -z-10" alt="Background" />
                    <div class="intro-control__inner mx-[30px]">
                        <div class="flex flex-col items-center justify-center">
                            <div class="text-white text-4xl font-Kuunari font-bold text center px-4">
                                <h1>BELUM PUNYA AKUN?</h1>
                            </div>
                            <div class="text-white text-sm font-Poppins text-center justify-center px-4">
...
<script>
    // Variabel global untuk menyimpan referensi ke elemen password yang sedang aktif (terlihat)
    // Kita menggunakan window._activePasswordElements agar bisa diakses oleh event listener global.
    // Awalan _ adalah konvensi untuk variabel yang bersifat "internal".
    ...
    function togglePassword(inputId, openId, closedId) {
        ...
        // Pemeriksaan dasar apakah elemen ditemukan
        if (!input || !openIcon || !closedIcon) {
            console.error("Satu atau lebih elemen tidak ditemukan untuk togglePassword:", inputId, openId, closedId);
            return;
        }
        ...
        input.type = isPassword ? 'text' : 'password'; // Toggle tipe input

        // Toggle kelas 'hidden' pada ikon
        if (isPassword) {
            openIcon.classList.toggle('hidden', true); // Tampilkan openIcon jika input jadi 'text'
            closedIcon.classList.toggle('hidden', false);
        } else {
            openIcon.classList.toggle('hidden', false); // Tampilkan openIcon jika input jadi 'text'
            closedIcon.classList.toggle('hidden', true);
        }
        // Tampilkan closedIcon jika input jadi 'password'

        // --- Integrasi untuk logika "click outside" ---
        if (input.type === 'text') {
            // Jika password sekarang terlihat, catat elemen-elemen ini sebagai yang "aktif"
            // Ini akan menimpa yang sebelumnya jika ada, jadi hanya satu set yang aktif untuk "click outside"
            window._activePasswordElements = {
                input,
                openIcon,
                closedIcon
            };
        } else {
            // Jika password sekarang tersembunyi, dan ini adalah elemen yang aktif, bersihkan catatannya
            if (window._activePasswordElements && window._activePasswordElements.input === input) {
                window._activePasswordElements = null;
            }
        }
    }

    // Tambahkan event listener ke seluruh dokumen untuk mendeteksi klik
    document.addEventListener('click', function(event) {
        // Periksa apakah ada elemen password yang sedang "aktif" (passwordnya terlihat)
        if (window._activePasswordElements) {
            const {
                input,
                openIcon,
                closedIcon
            ...
            // Kondisi: Apakah klik terjadi DI LUAR input DAN DI LUAR kedua ikonnya?
            // Kita periksa apakah target klik (event.target) adalah input, openIcon, atau closedIcon,
            // atau turunan dari elemen-elemen tersebut (menggunakan .contains()).
            const isClickInsideActiveElements =
                input.contains(event.target) ||
                openIcon.contains(event.target) ||
                closedIcon.contains(event.target);

            if (!isClickInsideActiveElements) {
                // Jika klik terjadi di LUAR area aktif (input dan ikonnya)
                // Dan pastikan input masih bertipe 'text' (password masih terlihat)
                if (input.type === 'text') {
                    // "Melakukan sesuatu": kembalikan tipe input ke password dan sesuaikan ikon
                    input.type = 'password';
                    openIcon.classList.toggle('hidden', false); // Tampilkan openIcon jika input jadi 'text'
                    closedIcon.classList.toggle('hidden', true);

                    // Setelah aksi, bersihkan catatan elemen aktif
                    window._activePasswordElements = null;
                    console.log('Password disembunyikan karena klik di luar area: ' + input.id);
                }
            }
        }
    });
