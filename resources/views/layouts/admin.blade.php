...
<body class="admin-body antialiased">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Add loading states to buttons (excluding Livewire forms)
            const buttons = document.querySelectorAll('button[type="submit"]:not([wire\\:submit])');
            buttons.forEach(button => {
                // Only apply to non-Livewire forms
                const form = button.closest('form');
                if (form && !form.hasAttribute('wire:submit') && !form.hasAttribute(
                    'wire:submit.prevent')) {
                    button.addEventListener('click', function() {
                        if (!this.disabled) {
                            ...
                            // Reset after 5 seconds if not handled by framework
                            setTimeout(() => {
                                if (this.disabled) {
